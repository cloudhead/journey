...
journey.delay = function (fn) { return setTimeout(fn, 0) };
...
journey.router = {
    ...
    resolve: function (request) {
        ...
        var found = this.routes.find(function (route) {
            ...
            if (match && constraints) {
                //journey.debug("Match on " + request.uri.full);
    ...
    dispatch: function (request, response, body, respond) {
        var resolved, route, params = request.uri.params;
    ...
    handler: function (request, response, body) {
        ...
        process.nextTick(function () { 
            ...
            journey.router.dispatch(request, response, body || "", function (outcome) {
                ...
                journey.log((new(Date)).toGMTString() + " -- " + request.method + " " + request.uri.full + " - HTTP " + code);
