    ...
    handler: function (request, response, body) {
        ...
        process.nextTick(function () { 
            ...
            journey.router.dispatch(request, response, body || "", function (outcome) {
                ...
                var status = (request.headers.accept === "application/json") ? outcome.status : 400;

                response.sendHeader(status, outcome.headers || {"Content-Type" : "text/html"});
                ...
                journey.log((new(Date)).toGMTString() + " -- " + request.method + " " + request.url.href + " - HTTP " + status);
