...
journey.router = {
    ...
    map: {
        ...
        route: function (/* variable arguments */) {
            ...
            return {
                ...
                bindResource: function (handler, success) {
                    var to;
        ...
        resources: function (object) {
            ...
            (function scan(obj, pattern) {
                Object.keys(obj).forEach(function (k) {
                    if (typeof(obj[k]) === 'function') {
                        (function (p, k) {
                            switch (k) {
                                case 'index':   return map.get(p.join('/'));
                                case 'show':    return map.get(p.concat([id]).join('/'));
                                case 'create':  return map.post(p.join('/'));
                                case 'destroy': return map.del(p.concat([id]).join('/'));
                                case 'update':  return map.put(p.concat([id]).join('/'));
                                default:        return map.route(p.concat([id]).join('/'));
                            }
                        })(pattern, k).bind(obj[k]);
                    } else {
                        if (pattern.length > 0) pattern.push(id);
                        scan(obj[k], pattern.concat(k));
                    }
        ...
        resource: function (name, definition) {
            var res, map;
    ...
    dispatch: function (request, body, respond) {
        ...
        try {
            ...
            if (e.status) return respond({ headers: e.headers || {}, status: e.status, body: e.message });
            else throw e;
        }

        route = this.route(request, respond);
        ...
        } else { return respond({ status: 400, body: "request not found" }) }
    },

    // A constructor of sorts, which returns a 'Routing context', in which the response
    // status is evaluated.
    route: function (req, respond) {
        return {
            request:   req,
            ...
            go: function (destination, params) {
                ...
                try {
                    ...
                    this.responder({ body: err.message || err, status: err.status || 500 });
                }
            }
        }
    }
...
journey.begin = function (request, body, callback) {
    ...
    process.nextTick(function () {
        ...
        journey.router.dispatch(request, body || "", function (outcome) {
            // Journey being a JSON-only server, we expect the 'Accept' header to be set accordingly
            var status  = outcome.status = (request.headers.accept === "application/json") ? outcome.status : 400,
                headers = outcome.headers || {"Content-Type":"text/html"},
                body    = outcome.body;

            promise.emit("success", outcome);

            if (callback) { callback(status, headers, body) }

            promise.emit("log", new(Date)().toGMTString() + " -- " + request.method + " " + request.url.href + " - HTTP " + status);
        });
    });
    return promise;
};

journey.draw = function (routes) {
    return journey.router.draw.call(journey.router, routes);
};
journey.route = journey.begin;
